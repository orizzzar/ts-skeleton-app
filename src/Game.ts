...
class Game {
  ...
  public startScreen() {
    ...
    const asteroidFileName =
      "./assets/images/SpaceShooterRedux/PNG/UI/buttonBlue.png";
    this.loadImage(asteroidFileName, this.writeStartButtonToStartScreen);
    // 4. add Asteroid image
    const startButtonFileName =
  ...
  public levelScreen() {
    // 1. load life images
    const lifeImageFileName =
      "./assets/images/SpaceShooterRedux/PNG/UI/playerLife1_blue.png";
    ...
    for (let i = 0; i < maxAsteroidsOnScreen; i++) {
      ...
      this.loadImage(
        asteroids[index],
        this.writeAsteroidImageToRandomLocationOnLevelScreen
      );
    }

    // 4. draw player spaceship
    const playerSpaceShipFileName =
   ...
   * @param {HTMLImageElement} img the loaded image object
   */
  private writeAsteroidImageToRandomLocationOnLevelScreen(
...
let init = function() {
  const Asteroids = new Game(document.getElementById(
    "canvas"
